...
- Name: features.spec.ts >> ETL Anything - Full Feature Verification (F001-F021) >> F008 - Dark Mode / Theme Toggle >> should have theme toggle in settings menu
- Location: e2e/features.spec.ts:116:9

# Error details

```
Error: locator.click: Error: "has" engine expects non-empty selector list
    ...
    at SelectorEvaluatorImpl._cached (<anonymous>:5048:20)
Call log:
  - waiting for locator('button:has svg').filter({ hasText: 'Menu' }).or(locator('[aria-label*="Settings"]')).first()
