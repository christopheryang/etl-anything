...
- Name: features.spec.ts >> ETL Anything - Full Feature Verification (F001-F021) >> F003 - File Upload UI for InputNode >> should drag input node to canvas
- Location: e2e/features.spec.ts:50:9
...
Call log:
  - waiting for locator('[data-sidebar-item="true"], .sidebar-item, button:has-text("Input"), div:has-text("Input")').first()
    - locator resolved to <div class="w-full h-screen bg-gray-50">…</div>
  ...
  - waiting for locator('.react-flow')
    - locator resolved to <div data-testid="rf__wrapper" class="react-flow bg-gray-50">…</div>
  - attempting move and up action
    2 × waiting for element to be visible and stable
      - element is visible and stable
      - scrolling into view if needed
      - done scrolling
      - <div class="text-xs text-gray-500">Data sources & uploads</div> from <div class="absolute left-4 top-20 bg-white rounded-lg shadow-lg p-4 w-64 z-10 pointer-events-none">…</div> subtree intercepts pointer events
    ...
    - retrying move and up action
      - waiting 100ms
    51 × waiting for element to be visible and stable
       - element is visible and stable
       - scrolling into view if needed
       - done scrolling
       - <div class="text-xs text-gray-500">Data sources & uploads</div> from <div class="absolute left-4 top-20 bg-white rounded-lg shadow-lg p-4 w-64 z-10 pointer-events-none">…</div> subtree intercepts pointer events
...
```ts
  ...
  2   | 
  3   | // Helper: wait for ReactFlow to be ready
  4   | async function waitForCanvasReady(page: Page) {
  5   |   await page.waitForSelector(".react-flow", { timeout: 10000 });
